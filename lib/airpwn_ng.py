...
class VictimParameters:
	...
	def hex_injection(self, injection):
		'''Generate hex string in packit format from injection string.'''
...
class Injector:
	...
	def inject(self, vicmac, rtrmac, vicip, svrip, vicport, svrport, acknum, seqnum, injection, TSVal, TSecr):
		...
		if ("mon" in self.interface):
			...
			### Do we need an except here?  What was the passing thought?
			except:
				pass
...
class PacketHandler:
	...
	def get_request(self, pkt):
		'''Extracts request payload as string from packet object if there is payload, otherwise returns None.'''
	...
	def proc_injection(self, vicmac, rtrmac, vicip, svrip, vicport, svrport, acknum, seqnum, request, cookie, TSVal, TSecr):
		...
		If no victims List is set, meaning it's in broadcast mode, it checks for the victim in PacketHandler.newvictims and gets the injection for it, if there is one, and injects it via Injector.inject().
		'''
		if (len(self.victims) == 0):
